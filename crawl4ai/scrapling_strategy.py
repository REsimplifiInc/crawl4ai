...
class AsyncScraplingCrawlerStrategy(AsyncCrawlerStrategy):
    ...
    async def start(self) -> None:
        ...
        self._session = factory(**session_kwargs)
    ...
    async def crawl(
        ...
        **kwargs: Any,
        ...
        response = await self._session.fetch(url, **fetch_kwargs)
        return self._response_to_crawl4ai(response, captures)
    ...
    def _build_fetch_kwargs(
        ...
        if (config.wait_for and wait_selector is None) or self._needs_page_action(
            config
        ):
    ...
    def _build_page_action(
        ...
        async def page_action(page: Any) -> dict[str, Any]:
            ...
            if config.wait_for and self._css_wait_selector(config.wait_for) is None:
            ...
            if config.js_code:
                result["js"] = await self._evaluate_scripts(page, config.js_code)
    ...
    async def _wait_for_condition(page: Any, condition: str, timeout_ms: int) -> None:
        if condition.startswith("js:"):
            ...
            return
        await page.locator(condition).first.wait_for(
    ...
    async def _evaluate_scripts(page: Any, scripts: str | list[str]) -> Any:
        ...
        for script in scripts if isinstance(scripts, list) else [scripts]:
            values.append(await page.evaluate(script))
